  ...
  it('sets actions state', () => {
    ...
    return new Promise((resolve) => {
      setTimeout(() => {
        ...
        expect(Math.ceil(Number(globalAction.state.executionTime))).to.be.greaterThan(
          ACTION_TIMEOUT
        );

        expect(modularAction.state.isExecuting).to.eq(false);
        expect(Math.ceil(Number(modularAction.state.executionTime))).to.be.greaterThan(
          ACTION_MODULAR_TIMEOUT
        );
